# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 16:26:21 2022
...
class Ego_ProHMR(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode):
        ...
        self.backbone = create_backbone(cfg) ## input [3,224,224] output feature*2048
...
#        self.renderer = SkeletonRenderer(self.cfg)
    ...
    def initialize(self, batch: Dict, conditioning_feats: torch.Tensor):
        ...
        smpl_params['body_pose'] = smpl_params['body_pose'].reshape(batch_size, -1, 3, 3)[:, :, :, :2].permute(0, 1, 3, 2).reshape(batch_size, 1, -1)[has_smpl_params]
        smpl_params['global_orient'] = smpl_params['global_orient'].reshape(batch_size, -1, 3, 3)[:, :, :, :2].permute(0, 1, 3, 2).reshape(batch_size, 1, -1)[has_smpl_params]
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        smpl_output,_ = self.smpl(global_orient=pred_smpl_params['global_orient'], body_pose=pred_smpl_params['body_pose'], betas=pred_smpl_params['betas'], pose2rot=False)
...
#            loss_keypoints_3d_exp = torch.tensor(0., device=device, dtype=dtype)
        ...
        smpl_params['body_pose'] = smpl_params['body_pose'].reshape(batch_size, -1, 3, 3)[:, :, :, :2].permute(0, 1, 3, 2).reshape(batch_size, 1, -1)
        smpl_params['global_orient'] = smpl_params['global_orient'].reshape(batch_size, -1, 3, 3)[:, :, :, :2].permute(0, 1, 3, 2).reshape(batch_size, 1, -1)
...
#        summary_writer.add_image('%s/predictions' % mode, predictions.transpose((2, 0, 1)), step_count)
    ...
    def training_step_discriminator(self, batch: Dict,
        ...
        gt_body_pose = gt_body['body_pose']
        gt_betas = gt_body['betas']
    ...
    def training_step(self, joint_batch: Dict, batch_idx: int, optimizer_idx: int) -> Dict:
        ...
        self.manual_backward(loss)
        optimizer.step()
        loss_disc = self.training_step_discriminator(batch, pred_smpl_params['body_pose'].reshape(batch_size * num_samples, -1), pred_smpl_params['betas'].reshape(batch_size * num_samples, -1), optimizer_disc)
